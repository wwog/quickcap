...
文档编写时，还未完成跨应用的通信，如果看到时没有实现，那可能采用的stdio的形式，由发起方进行标准输出的检测来判定程序执行的动态。另外，所有代码实现在lib crate,你也可以轻松的构建动态库来进行调用，例如nodejs的native module。或者平台动态库。如果项目后续没有完成，那可能不会完成,可能采用程序传参。
