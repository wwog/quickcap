...
impl CaptureResult {
    /// 零拷贝锁定数据（用于读取/渲染）
    pub fn lock_for_read(&self) -> Result<CaptureDataGuard, CaptureError> {
        let lock = self.pixel_buffer
...
impl<'a> CaptureDataGuard<'a> {
    ...
    pub fn as_slice(&self) -> &[u8] {
        ...
        let bytes_per_row = self.lock.buffer.bytes_per_row();
        let len = self.height * bytes_per_row;
    ...
    pub fn bytes_per_row(&self) -> usize {
        self.lock.buffer.bytes_per_row()
    }
}