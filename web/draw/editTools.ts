...
export class EditTools {
  ...
  set undoActive(active: boolean) {
    ...
    if (active) {
      ;(this.dom.querySelector('.edit-tool-item.undo .active') as HTMLDivElement).style.display =
        'block'
      ;(this.dom.querySelector('.edit-tool-item.undo .normal') as HTMLDivElement).style.display =
        'none'
    } else {
      ;(this.dom.querySelector('.edit-tool-item.undo .normal') as HTMLDivElement).style.display =
        'block'
  ...
  addListener = (items: {role: ItemRole; listener: (shape?: string) => void}[]) => {
    items.forEach((item) => {
  ...
  /**
   * 计算Windows平台下的工具栏位置
   * 确保工具栏显示在目标显示器范围内，避免超出屏幕边界
   */
  private calculatePositionForWindows = ({
    x,
    y,
    height,
    width,
    ...
    const domHeight = editToolHeight

    // 初始位置：工具栏右对齐选区，显示在选区下方
    let left = x + width - domWidth
    let top = y + height + editToolGap
    ...
    // 查找选区中心点所在的显示器
    let targetMonitor = this.monitors.find(
      monitor =>
        centerX >= monitor.x &&
    ...
    // 如果没有找到包含中心点的显示器，则选择距离最近的显示器
    if (!targetMonitor && this.monitors.length > 0) {
      let minDistance = Number.POSITIVE_INFINITY
      for (const monitor of this.monitors) {
        const monitorCenterX = monitor.x + monitor.width / 2
        const monitorCenterY = monitor.y + monitor.height / 2
        const distance = Math.sqrt(
          (centerX - monitorCenterX) ** 2 +
            (centerY - monitorCenterY) ** 2,
        )
    ...
    // 如果找到了目标显示器，调整位置确保工具栏在显示器范围内
    if (targetMonitor) {
      // 调整水平位置：防止工具栏超出显示器左右边界
      if (left < targetMonitor.x) {
        left = targetMonitor.x
      } else if (left + domWidth > targetMonitor.x + targetMonitor.width) {
        left = targetMonitor.x + targetMonitor.width - domWidth
      }

      // 调整垂直位置：防止工具栏超出显示器下边界
      if (top + domHeight > targetMonitor.y + targetMonitor.height) {
        // 优先尝试显示在选区上方
        if (y - domHeight - editToolGap >= targetMonitor.y) {
          top = y - domHeight - editToolGap
        } else if (y + height - domHeight - editToolGap >= targetMonitor.y) {
          // 其次尝试显示在选区内部
          top = y + height - domHeight - editToolGap
        } else {
          // 最后显示在显示器底部
          top = targetMonitor.y + targetMonitor.height
        }
      }

      // 防止工具栏超出显示器上边界
      if (top < targetMonitor.y) {
        top = targetMonitor.y
      }
    }
  ...
  private calculatePositionForMac = ({
    x,
    y,
    height,
    width,
    ...
    let top = 0
    let left = x + width - this.dom.clientWidth

    if (y + height + editToolHeight + editToolGap <= maxY) {
      // under the selection area
      top = y + height + editToolGap
    } else if (
      y + height + editToolHeight + editToolGap > maxY &&
      y - editToolHeight - editToolGap >= 0
    ) {
      // above the selection area
      top = y - editToolHeight - editToolGap
    } else {
      // inner the selection area
      top = y + height - editToolHeight - editToolGap
    }
    if (left < 0) {
      left = 0
    }
  ...
  render = (
    ...
    if (this.dom) {
      ...
      if (this._isWindows) {
        this.calculatePositionForWindows(selectRect)
      } else {
        this.calculatePositionForMac(selectRect)
      }
    }
  }
}
